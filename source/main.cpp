...
int SDL_main(int argc, char **argv) {
	...
	while (1) {
		...
		if (show_main_debug_ui) {
			...
			ImGui::Text("Position and Angle: %.2f %.2f %.2f", game.dynamicBody->GetPosition().x, game.dynamicBody->GetPosition().y, game.dynamicBody->GetAngle());
			ImGui::Text("AABB: %.2f, %.2f, %.2f, %.2f",
				game.dynamicBody->GetFixtureList()->GetAABB(0).upperBound.x,
				game.dynamicBody->GetFixtureList()->GetAABB(0).upperBound.y,
				game.dynamicBody->GetFixtureList()->GetAABB(0).lowerBound.x,
				game.dynamicBody->GetFixtureList()->GetAABB(0).lowerBound.y
			);
			ImGui::Separator();
